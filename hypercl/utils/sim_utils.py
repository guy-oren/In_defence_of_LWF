...
from mnets.hyper_model import HyperNetwork
...
def get_mnet_model(config, net_type, in_shape, out_shape, device, cprefix=None,
    ...
    """
    assert(net_type in ['mlp', 'resnet32', 'wide_resnet20_w8', 'wide_resnet20_w2', 'zenke', 'bio_conv_net'])
    ...
    elif net_type == 'wide_resnet20_w8':
        assert(len(out_shape) == 1)

        mnet = ResNet(in_shape=in_shape, num_classes=out_shape[0],
            verbose=True, n=3, k=8,
            no_weights=no_weights,
            #init_weights=None,
            use_batch_norm=assign(use_bn, True),
            bn_track_stats=assign(not bn_no_running_stats, True),
    ...
    elif net_type == 'wide_resnet20_w2':
        assert(len(out_shape) == 1)

        mnet = ResNet(in_shape=in_shape, num_classes=out_shape[0],
            verbose=True, n=3, k=2,
            no_weights=no_weights,
            #init_weights=None,
            use_batch_norm=assign(use_bn, True),
            bn_track_stats=assign(not bn_no_running_stats, True),
