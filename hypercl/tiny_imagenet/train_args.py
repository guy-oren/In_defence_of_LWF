...
def parse_cmd_arguments(mode='resnet32_tiny_imagenet', default=False, argv=None):
    ...
    Returns:
        (argparse.Namespace): The Namespace object containing argument names and
            values.
    """
    if mode == 'resnet_tiny_imagenet':
        description = 'Tiny Imagenet CL experiment using a Resnet'
    elif mode == 'zenke_tiny_imagenet':
        description = 'Tiny Imagenet CL experiment using the Zenkenet'
    else:
    ...
    if mode == 'resnet_tiny_imagenet':
        ...
        dout_dir = './out_resnet/run_' + \
            datetime.now().strftime('%Y-%m-%d_%H-%M-%S') + '_' + random_postfix
        ...
        cli.main_net_args(parser, allowed_nets=['resnet32', 'wide_resnet20_w8', 'wide_resnet20_w2'], show_batchnorm=False,
    ...
    if config.cl_scenario != 1:
        raise NotImplementedError('Tiny Imagenet experiments are currently only ' +
            'implemented for CL1.')
...
def general_options(parser):
    ...
    agroup.add_argument("--data_dir", type=str, required=True)
...
def special_cl_options(agroup):
    ...
    agroup.add_argument('--shuffle_order', action='store_true',
                        help='Determine if to shuffle the class order in cifar100')
