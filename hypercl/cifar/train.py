...
def test(task_id, data, mnet, hnet, device, shared, config, writer, logger,
         train_iter=None, task_emb=None, cl_scenario=None, test_size=None, is_val=False):
    ...
    with torch.no_grad():
        ...
        max_test_size = data.num_test_samples if not is_val else data.num_val_samples
        if test_size is None or test_size >= max_test_size:
            test_size = max_test_size
        else:
            # Make sure that we always use the same test samples.
            data.reset_batch_generator(train=False, test=True, val=True)
        ...
        while N_processed < test_size:
            ...
            if is_val:
                batch = data.next_val_batch(curr_bs)
            else:
    ...
    for i in range(training_iterations):
        ...
        if config.plateau_lr_scheduler:
            ...
            if i % iter_per_epoch == 0 and i > 0:
                ...
                logger.info('Computing val accuracy for plateau LR ' +
                ...
                test_acc, _ = test(task_id, data, mnet, hnet, device, shared,
                                   config, writer, logger, train_iter=i+1, is_val=True)
                mnet.train()
                if hnet is not None:
                    hnet.train()
...
def test_multiple(dhandlers, task_id, mnet, hnet, device, config, shared, writer,
                  logger):
    ...
    num_tasks = len(dhandlers[:task_id + 1])
    ...
    if config.cl_scenario == 1:
        ...
        for j in range(num_tasks):
            ...
            shared.summary['acc_during'][task_id][j] = test_acc
...
def run(config, experiment='resnet32'):
    """Run the training.
    ...
    """
    assert(experiment in ['resnet32', 'zenke', 'wide_resnet20_w8', 'wide_resnet20_w2'])
    ...
    for j in range(config.num_tasks):
        ...
        #                    logger)
        test_multiple(dhandlers, j, mnet, hnet, device, config, shared, writer,
                      logger)

        logger.info('### Accuracy of task %d / %d:  %.3f' % \
                    (j+1, config.num_tasks, shared.summary['acc_during'][j][j]))
        logger.info('### Finished training task: %d' % (j+1))

        # Backup results so far.
        tutils.save_summary_dict(config, shared, experiment)

    acc_during = [shared.summary['acc_during'][j][j] for j in range(config.num_tasks)]
    shared.summary['acc_avg_during'] = np.mean(acc_during)
    ...
    acc_final = [shared.summary['acc_during'][-1][j] for j in range(config.num_tasks)]
    shared.summary['acc_avg_final'] = np.mean(acc_final)
