...
def load_datasets(config, shared, logger, data_dir='../datasets'):
    ...
    dhandlers = get_split_cifar_handlers(data_dir, logger, use_one_hot=True,
        use_data_augmentation=augment_data, aug_ver=config.aug_ver, num_tasks=config.num_tasks, use_cifar10=config.use_cifar10,
                                         shuffle_order=config.shuffle_order, validation_size=config.val_size, seed=config.random_seed)
...
def get_main_model(config, shared, logger, device, no_weights=False):
    ...
    elif shared.experiment == 'wide_resnet20_w8':
