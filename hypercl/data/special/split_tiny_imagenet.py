...
"""
Split Tiny Imagenet Dataset
^^^^^^^^^^^^^^^^^^^^^^^^^^

The module :mod:`data.special.split_tiny_imagenet` contains a wrapper for data handlers
for the Split-TinyImageNet task.
"""
# FIXME The code in this module is mostly a copy of the code in the
# corresponding `split_mnist` module.
import numpy as np

from data.tiny_imagenet_data import TinyImagenetData

def get_split_tiny_imagenet_handlers(data_path, logger, use_one_hot=True, validation_size=0,
                             use_data_augmentation=False, aug_ver='aug-v1', num_tasks=6, shuffle_order=False):
    ...
    print('Creating data handlers for SplitTinyImagenet tasks ...')

    handlers = []

    tiny_imagenet_task_size = 200 // num_tasks

    tiny_imagenet_classes_order = np.arange(200)
    if shuffle_order:
        np.random.shuffle(tiny_imagenet_classes_order)

    all_labels = []
    for t, i in enumerate(range(0, num_tasks * tiny_imagenet_task_size, tiny_imagenet_task_size)):
        labels = tiny_imagenet_classes_order[range(i, i+tiny_imagenet_task_size)]
        logger.info("Task {} labels: {}".format(t, labels))
        handlers.append(SplitTinyImagenetData(data_path,
            use_one_hot=use_one_hot, validation_size=validation_size,
            use_data_augmentation=use_data_augmentation, aug_ver=aug_ver, labels=labels))

        all_labels.extend(labels)

    assert np.all(np.array(all_labels) == tiny_imagenet_classes_order)

    print('Creating data handlers for SplitTinyImagenet tasks ... Done')

    return handlers

class SplitTinyImagenetData(TinyImagenetData):
    """An instance of the class shall represent a single SplitTinyImagenet task.
    ...
    def __init__(self, data_path, use_one_hot=False, validation_size=1000,
        ...
        # Note, we continue to pretend to be a 200 class problem, such that
        ...
        print('Created SplitTinyImagenet task with labels %s and %d train, %d test '
